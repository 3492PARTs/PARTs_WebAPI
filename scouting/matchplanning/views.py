    ...
    def get(self, request, format=None):
        if has_access(request.user.id, auth_obj):
            try:
                ...
                return ret_message(
                    "An error occurred initializing match planning.",
                    True,
...
class PlanMatch(APIView):
    ...
    def get_match_information(self, match_id):
        ...
        for ts in teams:
            ...
            # Field Data
            team_results = scouting.field.util.get_field_results(t, self.request)
            field_cols = team_results["scoutCols"]
            field_answers = team_results["scoutAnswers"]

            # notes
            notes = TeamNotes.objects.filter(
                Q(void_ind="n") & Q(team_no=t) & Q(event=current_event)
            ).order_by("-time")
    ...
    def get(self, request, format=None):
        if has_access(request.user.id, auth_obj):
            try:
                req = self.get_match_information(
                ...
                return ret_message(
                    "An error occurred while getting match information.",
                    True,
                    app_url + self.endpoint,
                    exception=e,
                )
...
class LoadTeamNotes(APIView):
    ...
    def get_team_notes(self, team_no):
        ...
        notes = TeamNotes.objects.filter(
            Q(void_ind="n") & Q(team_no=team) & Q(event=current_event)
        ).order_by("-time")
        return notes
