...
class TokenObtainPairView(APIView):
    ...
    def post(self, request, format=None):
        try:
            ...
            return ret_message(
                "Invalid username or password.", True, app_url + self.endpoint, -1, e
            )
    ...
    def authenticate(self, request, **kwargs):
        ...
        try:
            ...
            user = UserModel.objects.get(
                (Q(email=username) | Q(username=username)) & Q(is_active=True)
            )
    ...
    def post(self, request):
        try:
            ...
            if serialized.is_valid():
                ...
                if serialized.data.get("password1", "t") != serialized.data.get(
                    "password2", "y"
                ):
                    return ret_message(
                        "Passwords don't match.", True, app_url + self.endpoint, -1
                    )

                user_data = serialized.validated_data

                try:
                    user = User.objects.get(email=user_data.get("email").lower())
                    return ret_message(
                ...
                cntx = {
                    ...
                    + "://"
                    + current_site.domain
                    + "/user/confirm/?pk={}&confirm={}".format(
                ...
                send_mail(
                    ...
                    message=render_to_string(
                        "email_templates/acc_active_email.txt", cntx
                    ).strip(),
                    html_message=render_to_string(
                        "email_templates/acc_active_email.html", cntx
                    ).strip(),
                ...
                for e in error_list:
                    error_str += "\n" + e
                return ret_message(
                    "An error occurred while creating user." + error_str,
                    True,
                    app_url + self.endpoint,
                    -1,
                    serialized._errors,
                ...
                if serializer.is_valid():
                    if "password" in serializer.validated_data:
                        ...
                        send_mail(
                            ...
                            message=render_to_string(
                                "email_templates/generic_email.txt", cntx
                            ).strip(),
                            html_message=render_to_string(
                                "email_templates/generic_email.html", cntx
                            ).strip(),
                            from_email="team3492@gmail.com",
                            recipient_list=[user.email],
                        )
                    ...
                    if request.user.is_superuser:  # only allow role editing if admin
                        ...
                        if "is_superuser" in serializer.validated_data:
                            user.is_superuser = serializer.validated_data[
                                "is_superuser"
                            ]
                    user.save()
...
class UserEmailConfirmation(APIView):
    ...
    def get(self, request):
        try:
            ...
            return ret_message(
                "Failed to activate user's account.",
                True,
                app_url + self.endpoint,
                exception=e,
            )
    ...
    def confirm_email(self, request, pk=None):
        ...
        try:
            ...
            if int(request.GET.get("confirm")) == user_confirm_hash:
                ...
                user.save()
                return redirect(
                    settings.FRONTEND_ADDRESS + "/login?page=activationConfirm"
                )
            else:
                ret_message(
                    "An error occurred while confirming the user's account.",
                    True,
                    app_url + self.endpoint,
                    user.id,
                )
                return redirect(
                    settings.FRONTEND_ADDRESS + "/login?page=activationFail"
                )
...
class UserEmailResendConfirmation(APIView):
    ...
    def post(self, request):
        try:
            ...
            return ret_message(
                "Failed to resend user confirmation email.",
                True,
                app_url + self.endpoint,
                exception=e,
            )
    ...
    def request_reset_password(self, request):
        ...
        try:
            ...
            if user.is_active:  # if the user has confirmed their email
                ...
                cntx = {
                    "user": user,
                    "url": settings.FRONTEND_ADDRESS
                    + "login/?page=resetConfirm&uuid={}&token={}".format(
...
class UserPasswordReset(APIView):
    ...
    def post(self, request):
        try:
            ...
            return ret_message(
                "Failed to reset password.", True, app_url + self.endpoint, exception=e
            )
    ...
    def reset_password(self, request):
        try:
            ...
            if default_token_generator.check_token(user, token):
                ...
                return ret_message(
                    "Invalid token or request timed out.",
                    True,
                    app_url + self.endpoint,
                    user.id,
                )
            ...
            e = e.strip("'")
            return ret_message(
                e + " missing from request but is required",
                True,
                app_url + self.endpoint,
                exception=e,
            )
    ...
    def forgot_username(self, request):
        ...
        try:
            ...
            send_mail(
                ...
                message=render_to_string(
                    "email_templates/forgot_username.txt", cntx
                ).strip(),
                html_message=render_to_string(
                    "email_templates/forgot_username.html", cntx
                ).strip(),
                from_email="team3492@gmail.com",
                recipient_list=[user.email],
            )
...
class Groups(APIView):
    ...
    def get(self, request, format=None):
        try:
            ...
            return ret_message(
                "An error occurred while getting groups.",
                True,
                app_url + self.endpoint,
                request.user.id,
                e,
...
class Alerts(APIView):
    ...
    def get(self, request, format=None):
        try:
            req = alerts.util.get_user_alerts(
                request.user.id, request.query_params.get("alert_comm_typ_id", None)
            )
            serializer = GetAlertsSerializer(req, many=True)
            return Response(serializer.data)
        except Exception as e:
            return ret_message(
                "An error occurred while getting user alerts.",
                True,
                app_url + self.endpoint,
                request.user.id,
                e,
